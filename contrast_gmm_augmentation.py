...
def main():
    gmm_augmentation(sys.argv[1], sys.argv[2])
    return

def gmm_augmentation(invol, mask):
    # Load the image and mask
    print('Loading image and mask...')
    n1_header = nib.load(invol).header
    t1_data = nib.load(invol).get_fdata()
    mask = nib.load(mask).get_fdata()
    n_components = 3 # default=3
    percentiles = (1,99) # default=(1, 99)
    std_means = (0.03, 0.06, 0.08) # default=(0.03, 0.06, 0.08)
    std_sigma = (0.012, 0.011, 0.015) # default=(0.012, 0.011, 0.015)
    # Clip percentiles and normalize the image
    t1_normalized = normalize_image(t1_data, clip_percentiles=False, pmin=percentiles[0], pmax=percentiles[1])

    print('Augmenting image...')
    new_image = generate_gmm_image(t1_normalized, mask=mask,
                                   n_components=n_components,
                                   std_means=tuple(std_means),
                                   std_sigma=tuple(std_sigma),
                                   p_mu=None, q_sigma=None,
                                   normalize=False)

    # Save the augmented image
    print('Saving augmented image...')
    img = nib.Nifti1Image(new_image, np.eye(4), n1_header)
    nib.save(img, str(invol[0:len(invol)-7])+'_caug.nii.gz')
