...
from utilities_new import MoCoDatasetRegression, tensorboard_correction, getSSIM, from_Img_to_kspace, \
                          kspace_cropcenter, from_kspace_toImg, tensorboard_correction_kspace
from pLoss.perceptual_loss import PerceptualLoss
# from models.ReconResNetV2 import ResNet
from models.KSPReconResNetv01 import ResNet
### ----------------------------------------------------- ###
...
log_path = './TBLogs'
trainID = 'ComplexRN5-ALL-BS1-ksp-core64-IXIT1-lr1e-3'
...
### ----------------------------------------------------- ###
model=ResNet(n_channels=channels, out_channels=channels, res_blocks=resblocks_, starting_nfeatures=64, 
            updown_blocks=2, is_relu_leaky=True, do_batchnorm=False, res_drop_prob=0.2,
            is_replicatepad=0, out_act="relu", forwardV=0, upinterp_algo='sinc', 
            post_interp_convtrans=False, is3D=False, img_out_mode=1, 
            fourier_norm_4imgout="ortho", under_replace=False, under_mask=None, 
            inner_norm_ksp=True, complex_weights=True)
    ...
    for idx, (cor, img, ssimtmp) in enumerate(tqdm(train_loader)):
        ...
        for jj in range(0, patches):
            ...
            with autocast(enabled=False):
                inpksp= from_Img_to_kspace(inp, sizeimg_= size_, coresize_=coresize_, 
                                           inner=True, outer=False, fullspc=False)
                gtksp = from_Img_to_kspace(gt,  sizeimg_= size_, coresize_=coresize_, 
                                           inner=True, outer=False, fullspc=False)
                out = model(inpksp.to(device))
                outimg = from_kspace_toImg(out, complex_=True)
                gtimg = from_kspace_toImg(gtksp, complex_=True)
            ...
            if idx % log_freq == 0:
                ...
                inpimg = from_kspace_toImg(inpksp, complex_=True)
