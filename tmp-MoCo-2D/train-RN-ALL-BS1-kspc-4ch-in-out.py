...
from utilities_new import MoCoDatasetRegression, tensorboard_correction, getSSIM, \
                          FourChannelComplexUncombined, Recombine4Ch, kspace_cropcenter, \
                          from_kspacecore_toImg, tensorboard_correction_kspace
...
device = torch.device("cuda:1") # if torch.cuda.is_available() and cuda else "cpu")
log_path = './TBLogs'
trainID = 'RN56-DEBUG-4CH-T1IXI-ReIm-DualModels'
...
traindata = glob('/pool/alex/Motion-Correction-3D/samples/output/T1-Guys-HH/train/*.nii.gz')
valdata = glob('/pool/alex/Motion-Correction-3D/samples/output/T1-Guys-HH/val/*.nii.gz')
...
modelOUT=ResNet(in_channels=channels,out_channels=channels,
	     res_blocks=resblocks_, starting_nfeatures=64, updown_blocks=2, is_relu_leaky=True, 			
	     do_batchnorm=False, res_drop_prob=0.2,
         out_act="tanh", forwardV=0,  # out_act="sigmoid"
	     upinterp_algo='convtrans', post_interp_convtrans=False, 
...
optimizerOUT = optim.Adam(modelOUT.parameters(), lr=learning_rate, amsgrad=True)
...
if checkpoint:
    ...
    modelOUT.load_state_dict(chk['state_dict'])
    optimizer.load_state_dict(chk['optimizer'])
    optimizerOUT.load_state_dict(chk['optimizerOUT'])
...
for epoch in range(start_epoch, num_epochs):
    ...
    for idx, (cor, img, ssimtmp) in enumerate(tqdm(train_loader)):
        ...
        for jj in range(0, patches):
            ...
            with autocast(enabled=True):
                inpksp = FourChannelComplexUncombined(inp, size_, coresize_)
                gtksp = FourChannelComplexUncombined(gt, size_, coresize_)
                outCORE = model(torch.unsqueeze(inpksp[0,0:2,...],0).to(device))
                outOUT = modelOUT(torch.unsqueeze(inpksp[0,2:,...],0).to(device))
                out = Recombine4Ch(outCORE.to(device), outOUT.to(device), size_, coresize_)
                loss = loss_func(out.to(device), gt)
                
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.step(optimizerOUT)
            scaler.update()
            ...
            if idx % log_freq == 0:
                ...
                # print(gtksp.size())
                gttmp = Recombine4Ch(torch.unsqueeze(gtksp[0,0:2,...],0), torch.unsqueeze(gtksp[0,2:,...],0), size_, coresize_)
                # inpimg = from_kspacecore_toImg(inpksp, size_, coresize_)
                # tensorboard_correction_kspace(tb_writer, inp, inpimg, outimg, gtimg, gt, epoch, 'train')
                tensorboard_correction(tb_writer, inp, out.detach(), gttmp, epoch, 'train')
                runningLoss = []
    ...
    if epoch % save_freq == 0:            
        checkpoint = {
                        ...
                        'state_dictOUT': modelOUT.state_dict(),
                        'optimizer': optimizer.state_dict(),
                        'optimizerOUT': optimizerOUT.state_dict(),
        ...
        with torch.no_grad():
            ...
            for i, (cor, img, ssimtmp) in enumerate(tqdm(val_loader)):
                ...
                for jj in range(0, patches):
                    ...
                    # ssim = getSSIM(gt.detach().cpu().numpy(), outimg.detach().cpu().numpy(), gt_flag=[True]*gtimg.shape[0], data_range=1)
