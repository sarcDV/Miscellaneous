...
from utilities_new import MoCoDatasetRegression, tensorboard_correction, getSSIM, split_dual_inner_outer
...
device = torch.device("cuda:3") # if torch.cuda.is_available() and cuda else "cpu")
log_path = './TBLogs'
trainID = 'RN56-ALL-BS1-dual-imgspace-debug-core64-v00'
save_path = './Results'
...
### ----------------------------------------------------- ###
model=ResNet(in_channels=channIN, out_channels=channels,
    ...
    for idx, (cor, img, ssimtmp) in enumerate(tqdm(train_loader)):
        ...
        for jj in range(0, patches):
            ...
            with autocast(enabled=True):
                # split_dual_inner_outer(intensor, sizeimg_, coresize_)
                inpd = split_dual_inner_outer(inp, size_, coresize_)
                out = model(inpd.to(device))
