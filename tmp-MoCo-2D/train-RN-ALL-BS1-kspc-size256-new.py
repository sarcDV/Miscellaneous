...
from utilities_new import MoCoDatasetRegression, tensorboard_correction, getSSIM, \
                          kspace_cropcenter, from_kspacecore_toImg, tensorboard_correction_kspace
...
log_path = './TBLogs'
trainID = 'RN56-ALL-BS1-ksp-Uncomb-core64-T1IXI'
    ...
    for idx, (cor, img, ssimtmp) in enumerate(tqdm(train_loader)):
        ...
        for jj in range(0, patches):
            ...
            with autocast(enabled=True):
                inpksp = kspace_cropcenter(inp, coresize_)
                gtksp = kspace_cropcenter(gt, coresize_)
                out = model(inpksp.to(device))
                outimg = from_kspacecore_toImg(out, size_, coresize_)
                gtimg = from_kspacecore_toImg(gtksp, size_, coresize_)
            ...
            for i, (cor, img, ssimtmp) in enumerate(tqdm(val_loader)):
                ...
                for jj in range(0, patches):
                    ...
                    with autocast(enabled=True):
                        inpksp = kspace_cropcenter(inp, coresize_)
                        gtksp = kspace_cropcenter(gt, coresize_)
                        out = model(inpksp.float().to(device))
                        outimg = from_kspacecore_toImg(out, size_, coresize_)
                        gtimg = from_kspacecore_toImg(gtksp, size_, coresize_)
                        loss = loss_func(outimg.to(device), gtimg.to(device))
                        # out = model(inp)
