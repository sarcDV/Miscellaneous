...
log_path = './TBLogs'
trainID = 'RN56-DEBUG-4CH-T1IXI-ReIm-SingleModel'
    ...
    for idx, (cor, img, ssimtmp) in enumerate(tqdm(train_loader)):
        ...
        for jj in range(0, patches):
            ...
            with autocast(enabled=True):
                ...
                # print(inpksp.size())
                outtmp = model(inpksp.to(device).float())
                out = Recombine4Ch(torch.unsqueeze(outtmp[0,0:2,...],0), 
                            torch.unsqueeze(outtmp[0,2:,...],0), size_, coresize_)
